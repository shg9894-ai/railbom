...
def create_formation(vehicle_id: int, body: FormationCreate, _=Depends(require_user)):
    if not vehicle_repo.get_vehicle_by_id(vehicle_id):
        raise HTTPException(status_code=404, detail="차종을 찾을 수 없습니다.")
...
def create_formations_bulk(vehicle_id: int, body: FormationBulkCreate, _=Depends(require_user)):
    if not vehicle_repo.get_vehicle_by_id(vehicle_id):
        raise HTTPException(status_code=404, detail="차종을 찾을 수 없습니다.")
...
def update_formation(formation_id: int, body: FormationUpdate, _=Depends(require_user)):
...
def delete_formation(formation_id: int, _=Depends(require_user)):
    if not formation_repo.get_formation_by_id(formation_id):
        raise HTTPException(status_code=404, detail="편성을 찾을 수 없습니다.")
